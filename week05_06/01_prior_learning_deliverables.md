...
- [x] 조건문 없이 처리할 수 있는 구조적 대안에 대한 아이디어를 작성해 주세요.
        ...
        이동을 위와 같이 상/하/좌/우/좌상/우상/좌하/우하 8개 방위로 분리하고, 그 조합으로 이동 경로를 포현하게,
        각 이동 경로를 비교해 이동 가능 여부, 포기(捕棋) 가능 여부를 판별하도록 하면
        조건문을 대폭 감소시키고,
        그 이동 경로에서 포는 포를 검사하게? 결국 if 문 완벽한 배제는 불가능하겠지만 대폭 감소는 가능할 것으로 예상 
