...
<div class="max-width-md mx-auto pt-10 px-5"><h1 class="display-2 pt-5 pb-7 text-bold">📘 사이클2 - 토론 활동</h1> <div class="mt-5"><div><div class="tui-editor-contents"><h1>사이클2: 토론 활동</h1>
<blockquote>
<p><strong>지금까지의 규칙을 리뷰하고, 사전학습에서 분석한 "함께 일어나는 작업"을 꺼내 비교해 "트랜잭션 경계"에 대한 새 규칙을 만든다. 지금까지의 규칙과 통합해 규칙을 업데이트한다.</strong></p>
</blockquote>
<hr>
<h2>지금까지의 규칙 리뷰</h2>
<p>먼저 지금까지(이전 미션 + 이번 미션의 사이클1) 만든 규칙을 리뷰한다:</p>
<ul>
<li>잘 작동한 규칙은? (어떤 결정에서 도움이 됐는가)</li>
<li>수정이 필요한 규칙은? (코드에 적용하다 막혔거나 모호했던 부분)</li>
<li>트랜잭션 경계 상황에서도 그대로 유효한 규칙은?</li>
<li>새로 추가할 규칙은?</li>
</ul>
<blockquote>
<p>4번 토론 주제와 이어진다. 이 리뷰에서 정리한 결과가 레벨2 최종 규칙의 출발점이 된다.</p>
</blockquote>
<hr>
<h2>토론 활동 주제</h2>
<table>
...
<tr>
<td>1</td>
<td>어떤 작업들이 실패해도 괜찮은가?</td>
</tr>
<tr>
<td>2</td>
<td>대기 전환과 순번 재정렬은 하나로 묶어야 하는가?</td>
</tr>
<tr>
<td>3</td>
<td>트랜잭션이 실패하면 사용자에게 "다시 시도하세요"로 충분한가?</td>
</tr>
<tr>
<td>4</td>
<td>지금까지 만든 규칙 중 이 기능에 적용되지 않는 것은 무엇인가?</td>
</tr>
</tbody>
</table>
<hr>
<h2>토론 활동 규칙</h2>
<ul>
<li>사전학습에서 분석한 <strong>데이터 상태</strong>를 근거로 사용한다 ("이렇게 묶는 게 맞을 것 같다"가 아니라 "이걸 묶지 않으면 데이터가 이렇게 된다")</li>
<li><code data-backticks="1">@Transactional</code>을 어디에 붙이는가가 아니라 <strong>왜 이것들이 함께 일어나야 한다고 판단했는가</strong>에 집중한다</li>
<li>의견 충돌은 좋은 것, 성급하게 합의하지 않는다</li>
<li>4번 주제에서는 지금까지의 규칙 문서를 다시 펼쳐 놓고 본다</li>
</ul>
<hr>
<h2>토론 활동 결과물</h2>
<p>다음을 만든다.</p>
<ol>
<li><strong>트랜잭션 경계 규칙</strong> — 4가지 토론 주제에 대응하는 그룹 규칙
<ul>
<li>함께 묶어야 하는 작업의 판단 기준</li>
<li>묶지 않아도 되는 작업의 판단 기준</li>
<li>트랜잭션 실패 시 사용자 경험 설계</li>
<li>지금까지의 규칙 중 이 기능에 적용/미적용한 항목과 그 이유</li>
</ul>
</li>
<li><strong>레벨2 최종 규칙</strong> — 지금까지의 규칙 + 이 사이클의 규칙을 통합한 단일 문서</li>
</ol>
<hr>
<h2>규칙 작성 형식</h2>
                <pre id="default-daj154zc8l" class="hljs-code-block overflow-hidden relative d-flex mt-3 mb-8">                    <button type="button" class="clipboard-btn v-btn v-btn--flat v-btn--icon v-btn--round theme--light v-size--default absolute right-10 top-10 z-1 grey--text text--darken-3">
                   ...
                   language-plaintext hljs">(If-Then) 만약 [데이터 변경 상황] → 우리는 [경계 결정 + 이유]

(우선순위) [경계를 결정할 때] 순서: 1) ... 2) ... 3) ...
...
<h3>규칙 예시</h3>
                <pre id="default-ce9h1iehlc" class="hljs-code-block overflow-hidden relative d-flex mt-3 mb-8">                    <button type="button" class="clipboard-btn v-btn v-btn--flat v-btn--icon v-btn--round theme--light v-size--default absolute right-10 top-10 z-1 grey--text text--darken-3">
                      <span class="v-btn__content">
                        <i aria-hidden="true" class="v-icon notranslate mdi mdi-clipboard theme--light"></i>
                      </span>
                    </button>
                    <code class="relative pa-4 lh-1-3 text-normal width-100 ls-0
                   language-plaintext hljs">(If-Then) 만약 한 변경이 실패했을 때 다른 변경이 사용자에게 잘못된 상태를 보이게 한다면
...
<h2>토론 활동 종료 조건</h2>
<ul>
<li class="task-list-item" data-te-task="">트랜잭션 경계 규칙이 작성됨</li>
<li class="task-list-item" data-te-task="">지금까지의 규칙 + 이 사이클의 규칙을 <strong>통합한 규칙</strong>이 있음</li>
<li class="task-list-item" data-te-task="">각 규칙이 <strong>코드에서 확인 가능한 신호</strong>를 포함 (트랜잭션 경계, 함께 변경되는 데이터, 실패 시 응답 등)</li>
</ul>
<hr>
<h2>산출물</h2>
<blockquote>
...
<ul>
<li class="task-list-item" data-te-task="">규칙 업데이트 문서(그룹)</li>
</ul>
</div></div></div></div>

</details>

<details><summary><h2>지금까지의 규칙</h2></summary>
...
- `API 설계 조건`
    - 클라이언트가 요청한 `리소스` 를 제공하기 위한 로직을 설계한다는 결정에서 기준점이 됨
- `서버/클라이언트 검증 책임`
    - 클라이언트는 전달받은 데이터와 저장된 데이터를 기반으로 유효성을 검증하고 응답
- `좋은 API, 동작하는 API`
    - 메서드와 리소스만으로 요청의 목적과 응답에 필요한 정보를 드러내고, `올바른` 응답을 제공
- `에러 응답의 수신자/내용`
    - 에러의 수신자는 클라이언트로 가정하고, 클라이언트가 사용자에게 전달할 내용을 가공
- `테스트가 보호하는 대상`
    - 스스로 작성한 `도메인 로직` 과 결과인 `사용자 응답` 을 보호

### <li>수정이 필요한 규칙은? (코드에 적용하다 막혔거나 모호했던 부분)</li>

- 인수 테스트와 E2E 테스트의 명확한 정의와 적용 규칙
- 관리자/사용자 API 분리 - 인증과 인가가 없는 상태에서 분리의 실효성 재고
